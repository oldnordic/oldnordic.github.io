...
**The V3 native backend was a dead end.** v0.5.0 and v4.0 both had a "native" binary format backend alongside SQLite. It was faster for raw writes but couldn't match SQLite's tooling ecosystem (any sqlite3 CLI can inspect the database, run ad-hoc queries, check integrity). Removed in v4.7.2. The 80/20 was: SQLite is 80% as fast and 20x more debuggable.
