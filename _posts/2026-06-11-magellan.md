...
**The V2 native backend was an experiment that didn't pan out.** v0.5.0 introduced a binary format backend alongside SQLite. It was faster for raw writes but couldn't match SQLite's tooling ecosystem (any sqlite3 CLI can inspect the database, run ad-hoc queries, check integrity). The V2 was removed. The V3 native backend is still in the codebase and works — but SQLite handles everything the stack needs, so there's no reason to switch. The 80/20: SQLite is 80% as fast and 20x more debuggable.
